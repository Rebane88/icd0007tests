...
test('Contains valid tokens', function () {
    global $projectDir, $username, $secret;

    $dict = tokensAsDict(getTokenFilePath($projectDir));

    $errors = validateTokens($dict, $username, $secret);

    assertThat(count($errors), is(0), join("\n", $errors));
});
...
function getTokenFilePath($path): string {
    ...
    $dataFile = "$path/ex7/tokens.txt";

    if (!file_exists($dataFile)) {
        die("can't find ex7/tokens.txt from $path" . PHP_EOL);
    }
